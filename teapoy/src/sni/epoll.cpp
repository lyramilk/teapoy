...
namespace lyramilk{ namespace teapoy{
	...
	class server_poll:public lyramilk::io::aiopoll
	{
	  ...
	  public:
		...
		{
			MILK_CHECK_SCRIPT_ARGS_LOG(log,lyramilk::log::warning,__FUNCTION__,params,0,lyramilk::data::var::t_user);
			const void* pobj = params[0].userdata(lyramilk::script::engine::s_user_nativeobject());
			if(!pobj) return false;
			lyramilk::netio::aiolistener* psrv = (lyramilk::netio::aiolistener*)pobj;
			return lyramilk::io::aiopoll::add(psrv);
		}

		lyramilk::data::var active(lyramilk::data::var::array params,lyramilk::data::var::map env)
		{
			if(params.size() == 0){
				lyramilk::io::aiopoll::active();
				log(lyramilk::log::debug,__FUNCTION__) << D("启动了%d个线程",size()) << std::endl;
			}else{
				MILK_CHECK_SCRIPT_ARGS_LOG(log,lyramilk::log::warning,__FUNCTION__,params,0,lyramilk::data::var::t_int32);
				int i = params[0];
		...
		lyramilk::data::var wait(lyramilk::data::var::array params,lyramilk::data::var::map env)
		{
